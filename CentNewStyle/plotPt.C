...
//supplemental info on plot:
    if(whichPlot==0){
		...
		lat->SetTextFont(42);
        lat->SetTextSize(ltxSetTextSize2);
        lat->DrawLatex(xsec_ltxText1_xStart,xsec_ltxText1_yStart-0.65,Form("Global uncert. B_{s}^{0}: #pm %.1f %%",glbSystUp));
		lat->DrawLatex(xsec_ltxText1_xStart,xsec_ltxText1_yStart-0.70,Form("Global uncert. B^{+}: #pm %.1f %%",glbSystUpBp));
	...
	legXSec->Draw();
        ...
        lat->SetTextFont(42);
        lat->SetTextSize(ltxSetTextSize2);
        lat->DrawLatex(legRatio_xLowStart-0.08,legRatio_y-0.1,Form("Global uncert.: #pm %.1f %%",glbSystDown));
	...
	if(drawRef)
	  {
	    TLegend *legRatioRef = new TLegend(legRatioRef_xLowStart,legRatioRef_y-0.05,legRatioRef_xLowEnd,legRatio_y+0.10,NULL,"brNDC");
